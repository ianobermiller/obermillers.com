...
import { dateRangeAlignWeek, toISODateString } from "./date";
...
export function autoColor(
  ...
  const range = dateRangeAlignWeek(new Date(calendar.startDate), new Date(calendar.endDate)).map(
    (date) => ({
      date,
  ...
  return categories.map((category) => {
    const adjacentColors = new Set(
      Array.from(adjacentCategoriesById.get(category.id) ?? [], (id) => colorByCategoryId.get(id)),
    );
    const availableColors = COLORS.filter((color) => !adjacentColors.has(color));
    const color = availableColors.length > 0 ? wrapAt(availableColors, ++j) : wrapAt(COLORS, ++j);
    return { ...category, color };
  });
}

function wrapAt<T>(arr: T[], index: number): T {
  const value = arr[wrap(arr.length, index)];
  if (value === undefined) {
    throw new Error("Cannot wrap an empty array");
  }
  return value;
}
