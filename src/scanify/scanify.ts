...
export type ScanPreset = ScanOptions & { label: string };
...
} as const satisfies Record<string, ScanPreset>;
...
function buildToneCurve(o: ScanOptions): Uint8Array {
...
type LightField = { fx: Float32Array; fy: Float32Array };

// Uneven lamp brightness, approximated as a separable field so it stays cheap.
function buildLightField(w: number, h: number, o: ScanOptions, rng: Rng): LightField {
...
function buildRowShifts(h: number, o: ScanOptions, rng: Rng): Int16Array {
...
function paperColor(o: ScanOptions): string {
  const warm = o.warmth;
...
function signedJitter(rng: Rng, max: number): number {
...
function layoutPage(w: number, h: number, o: ScanOptions, rng: Rng): PageLayout {
  return {
...
function addDebris(
  ...
  for (let i = 0; i < hairs; i++) {
    ...
    ctx.moveTo(x0, y0);
    ctx.bezierCurveTo(
      x0 + between(rng, -len, len),
      y0 + between(rng, -len, len),
      x0 + between(rng, -len, len),
      y0 + between(rng, -len, len),
      x0 + between(rng, -len, len),
      y0 + between(rng, -len, len),
...
export async function scanifyPage(
  src: HTMLCanvasElement,
  options: ScanOptions,
  seed: number,
): Promise<Blob> {
