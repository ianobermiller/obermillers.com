 ...
 * @param {HTMLImageElement} detectImg - Resized image for face detection (can be null)
 * @param {number} scale - Scale factor between detectImg and img
 * @param {boolean} useFaceDetection - Whether to use face detection
 * @returns {Promise<HTMLImageElement>} Cropped passport-sized image
 */
export async function cropImage(img, detectImg, scale, useFaceDetection = true) {
    ...
    // Face detection on resized image (only if enabled)
    if (useFaceDetection && detectImg) {
        try {
